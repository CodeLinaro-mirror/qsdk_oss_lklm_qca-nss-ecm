...
#define ECM_PPE_IPV6_PATH "net/ecm/ecm_ppe_ipv6"	/* Default path for sysctl */

static struct ctl_table_header *ecm_ppe_ipv6_ctl_table_header;	/* Sysctl table header */
 ...
 */
static int ecm_ppe_ipv6_accel_limit_mode_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To limit the number of acceleration
	 * echo 1 > /proc/sys/net/ecm/ecm_ppe_ipv6/accel_limit_mode
	 *
	 * To disable acceleration limitations
	 * echo 0 >  /proc/sys/net/ecm/ecm_ppe_ipv6/accel_limit_mode
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv6/accel_limit_mode
	 */
	 ...
	 */
	current_val = ecm_ppe_ipv6_accel_limit_mode;
	 ...
	 */
	if (ecm_ppe_ipv6_accel_limit_mode & ~(ECM_FRONT_END_ACCEL_LIMIT_MODE_FIXED | ECM_FRONT_END_ACCEL_LIMIT_MODE_UNLIMITED)) {
		DEBUG_ERROR("ecm_ppe_ipv6_accel_limit_mode = %x bad\n", ecm_ppe_ipv6_accel_limit_mode);
		ecm_ppe_ipv6_accel_limit_mode = current_val;
		return -EINVAL;
	}
 ...
 */
static int ecm_ppe_ipv6_driver_fail_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To limit number of PPE driver interaction failure (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv6/driver_fail_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv6/driver_fail_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv6_driver_fail_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv6_driver_fail_limit_default < 0) {
		ecm_ppe_ipv6_driver_fail_limit_default = current_val;
 ...
 */
static int ecm_ppe_ipv6_nack_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To handle number of nacK from PPE (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv6/nack_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv6/nack_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv6_nack_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv6_nack_limit_default < 0) {
		ecm_ppe_ipv6_nack_limit_default = current_val;
...
/*
 * ecm_ppe_ipv6_no_action_limit_default_handler()
 * 	Proc handler to set default no action limit
 */
static int ecm_ppe_ipv6_no_action_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To set no action seen from frontend (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv6/no_action_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv6/no_action_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv6_no_action_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv6_no_action_limit_default < 0) {
		ecm_ppe_ipv6_no_action_limit_default = current_val;
		DEBUG_ERROR("Invalid input, valid input positive integer\n");
		return -EINVAL;
	}
...
/*
 * ecm_ppe_ipv6_vlan_passthrough_enable_handler()
 * 	Proc handler to enable/disable VLAN passthrough
 */
static int ecm_ppe_ipv6_vlan_passthrough_enable_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * Enable VLAN passthrough
	 * echo 1 > /proc/sys/net/ecm/ecm_ppe_ipv6/vlan_passthrough_set
	 *
	 * Disable VLAN passthrough
	 * echo 0 > /proc/sys/net/ecm/ecm_ppe_ipv6/vlan_passthrough_set
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv6/vlan_passthrough_set
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv6_vlan_passthrough_enable;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if ((ecm_ppe_ipv6_vlan_passthrough_enable != 0) && (ecm_ppe_ipv6_vlan_passthrough_enable != 1)) {
		ecm_ppe_ipv6_vlan_passthrough_enable = current_val;
...
static struct ctl_table ecm_ppe_ipv6_ctl_table[] = {
	{
		.procname	= "accel_limit_mode",
		.data		= &ecm_ppe_ipv6_accel_limit_mode,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv6_accel_limit_mode_handler,
	},
	{
		.procname	= "driver_fail_limit_default",
		.data		= &ecm_ppe_ipv6_driver_fail_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv6_driver_fail_limit_default_handler,
	},
	{
		.procname	= "nack_limit_default",
		.data		= &ecm_ppe_ipv6_nack_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv6_nack_limit_default_handler,
	},
	{
		.procname	= "no_action_limit_default",
		.data		= &ecm_ppe_ipv6_no_action_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv6_no_action_limit_default_handler,
	},
	{
		.procname	= "vlan_passthrough_set",
		.data		= &ecm_ppe_ipv6_vlan_passthrough_enable,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv6_vlan_passthrough_enable_handler,
	},
};
...
int ecm_ppe_ipv6_init(struct dentry *dentry)
{
	...
	ecm_ppe_ipv6_ctl_table_header = register_sysctl(ECM_PPE_IPV6_PATH, ecm_ppe_ipv6_ctl_table);
	if (!ecm_ppe_ipv6_ctl_table_header) {
		DEBUG_ERROR("Failed to create ecm ppe ipv4 directory in sysctl\n");
		return -1;
	}

	if (!ecm_debugfs_create_dir("ecm_ppe_ipv6", dentry, &ecm_ppe_ipv6_dentry)) {
		DEBUG_ERROR("Failed to create ecm ppe ipv6 directory in debugfs\n");
		unregister_sysctl_table(ecm_ppe_ipv6_ctl_table_header);
		return -1;
	}

	ecm_debugfs_create_u32("accelerated_count", S_IRUGO, ecm_ppe_ipv6_dentry,
				(u32 *)&ecm_ppe_ipv6_accelerated_count);
	ecm_debugfs_create_u32("pending_accel_count", S_IRUGO, ecm_ppe_ipv6_dentry,
				(u32 *)&ecm_ppe_ipv6_pending_accel_count);
	ecm_debugfs_create_u32("pending_decel_count", S_IRUGO, ecm_ppe_ipv6_dentry,
				(u32 *)&ecm_ppe_ipv6_pending_decel_count);

	if (!ecm_debugfs_create_file("accel_cmd_avg_millis", S_IRUGO, ecm_ppe_ipv6_dentry,
	...
	if (!ecm_debugfs_create_file("decel_cmd_avg_millis", S_IRUGO, ecm_ppe_ipv6_dentry,
	...
	ecm_ppe_ported_ipv6_init(ecm_ppe_ipv6_dentry);

	if (!ecm_debugfs_create_file("stats_request_counter", S_IRUGO, ecm_ppe_ipv6_dentry,
...
#ifdef ECM_NON_PORTED_SUPPORT_ENABLE
	ecm_ppe_non_ported_ipv6_init(ecm_ppe_ipv6_dentry);
#endif
...
task_cleanup_1:
	ecm_debugfs_remove_recursive(ecm_ppe_ipv6_dentry);
	unregister_sysctl_table(ecm_ppe_ipv6_ctl_table_header);
	return -1;
}
