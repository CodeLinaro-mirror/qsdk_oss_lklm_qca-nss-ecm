									...
									struct nf_conn *ct, struct sk_buff *skb)
{
	...
	for (list_index = from_ifaces_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
		...
		case ECM_DB_IFACE_TYPE_IPSEC_TUNNEL:
			...
			/*
			 * TODO: Add support for IPsec for PPE frontend.
			 */
			DEBUG_TRACE("%px: IPSEC is not supported in PPE: %d (%s)\n", feci, ii_type, ii_name);
			rule_invalid = true;
#endif
			break;
 ...
 */
struct ecm_front_end_connection_instance *ecm_ppe_non_ported_ipv4_connection_instance_alloc(bool can_accel,
								int protocol,
