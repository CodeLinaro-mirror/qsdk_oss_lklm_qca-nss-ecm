...
#define ECM_PPE_IPV4_PATH "net/ecm/ecm_ppe_ipv4"	/* Default path for sysctl */

static struct ctl_table_header *ecm_ppe_ipv4_ctl_table_header;	/* Sysctl table header */
...
static int ecm_ppe_ipv4_accel_limit_mode_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	 ...
	 * echo 1 > /proc/sys/net/ecm/ecm_ppe_ipv4/accel_limit_mode
	 *
	 * To disable acceleration limitations
	 * echo 0 > /proc/sys/net/ecm/ecm_ppe_ipv4/accel_limit_mode
	 *
	 * To read status:
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv4/accel_limit_mode
	 */
	 ...
	 */
	current_val = ecm_ppe_ipv4_accel_limit_mode;
	 ...
	 */
	if (ecm_ppe_ipv4_accel_limit_mode & ~(ECM_FRONT_END_ACCEL_LIMIT_MODE_FIXED | ECM_FRONT_END_ACCEL_LIMIT_MODE_UNLIMITED)) {
		DEBUG_ERROR("ecm_ppe_ipv4_accel_limit_mode = %x bad\n", ecm_ppe_ipv4_accel_limit_mode);
		ecm_ppe_ipv4_accel_limit_mode = current_val;
		return -EINVAL;
	}
...
static int ecm_ppe_ipv4_driver_fail_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	 ...
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv4/driver_fail_limit_default
	 *
	 * To read status:
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv4/driver_fail_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv4_driver_fail_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv4_driver_fail_limit_default < 0) {
		ecm_ppe_ipv4_driver_fail_limit_default = current_val;
...
static int ecm_ppe_ipv4_nack_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	 ...
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv4/nack_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv4/nack_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv4_nack_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv4_nack_limit_default < 0) {
		ecm_ppe_ipv4_nack_limit_default = current_val;
...
static int ecm_ppe_ipv4_no_action_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	 ...
	 * echo x > /proc/sys/net/ecm/ecm_ppe_ipv4/no_action_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv4/no_action_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv4_no_action_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_ppe_ipv4_no_action_limit_default < 0) {
		ecm_ppe_ipv4_no_action_limit_default = current_val;
		DEBUG_ERROR("Invalid input, valid input positive integer\n");
		return -EINVAL;
	}
...
static int ecm_ppe_ipv4_vlan_passthrough_enable_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	 ...
	 * echo 1 > /proc/sys/net/ecm/ecm_ppe_ipv4/vlan_passthrough_set
	 *
	 * Disable VLAN passthrough
	 * echo 0 > /proc/sys/net/ecm/ecm_ppe_ipv4/vlan_passthrough_set
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_ppe_ipv4/vlan_passthrough_set
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_ppe_ipv4_vlan_passthrough_enable;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if ((ecm_ppe_ipv4_vlan_passthrough_enable != 0) && (ecm_ppe_ipv4_vlan_passthrough_enable != 1)) {
		ecm_ppe_ipv4_vlan_passthrough_enable = current_val;
...
static struct ctl_table ecm_ppe_ipv4_ctl_table[] = {
	{
		.procname	= "accel_limit_mode",
		.data		= &ecm_ppe_ipv4_accel_limit_mode,
		.maxlen		= sizeof(uint32_t),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv4_accel_limit_mode_handler,
	},
	{
		.procname	= "driver_fail_limit_default",
		.data		= &ecm_ppe_ipv4_driver_fail_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv4_driver_fail_limit_default_handler,
	},
	{
		.procname	= "nack_limit_default",
		.data		= &ecm_ppe_ipv4_nack_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv4_nack_limit_default_handler,
	},
	{
		.procname	= "no_action_limit_default",
		.data		= &ecm_ppe_ipv4_no_action_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_ppe_ipv4_no_action_limit_default_handler,
	},
	{
		.procname	= "vlan_passthrough_set",
		.data		= &ecm_ppe_ipv4_vlan_passthrough_enable,
		.maxlen		= sizeof(int),
		.mode		= 0644,
...
int ecm_ppe_ipv4_init(struct dentry *dentry)
{
	...
	ecm_ppe_ipv4_ctl_table_header = register_sysctl(ECM_PPE_IPV4_PATH, ecm_ppe_ipv4_ctl_table);
	if (!ecm_ppe_ipv4_ctl_table_header) {
