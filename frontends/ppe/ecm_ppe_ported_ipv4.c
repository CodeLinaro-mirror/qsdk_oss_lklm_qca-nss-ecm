...
process_next_iface_flow:
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
				...
				ecm_ppe_stats_v4_inc(feci, ECM_PPE_STATS_V4_EXCEPTION_PORTED, ECM_PPE_STATS_V4_EXCEPTION_PORTED_FROM_IFACE_DSA_QINQ_UNSUPPORTED);
				break;
			}
			 ...
			 */
			pd4rc->vlan_rule.primary_vlan.ingress_vlan_tag = dsa_vlan_value;
...
process_next_iface_return:
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
				rule_invalid = true;
				DEBUG_TRACE("%px: DSA/VLAN - Q-in-Q vlan unsupported\n", feci);
				ecm_ppe_stats_v4_inc(feci, ECM_PPE_STATS_V4_EXCEPTION_PORTED, ECM_PPE_STATS_V4_EXCEPTION_PORTED_TO_IFACE_DSA_QINQ_UNSUPPORTED);
				break;
			}
			 ...
			 * Ready to write the DSA VLAN rule
			 */
			pd4rc->vlan_rule.primary_vlan.egress_vlan_tag = dsa_vlan_value;
			pd4rc->valid_flags |= PPE_DRV_V4_VALID_FLAG_VLAN;
			interface_type_counts[ECM_DB_IFACE_TYPE_VLAN]++;

			ecm_db_iface_dsa_address_get(ii, to_ppe_iface_address);
			ether_addr_copy((uint8_t *)to_ppe_iface_address, dsa_info.address);
			DEBUG_TRACE("%px: DSA rule config found with vlan tag: 0x%x\n", feci, dsa_vlan_value);
#else
			rule_invalid = true;
			DEBUG_TRACE("%px: DSA interface is not supported\n", feci);
