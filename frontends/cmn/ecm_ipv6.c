		...
		case ECM_DB_IFACE_TYPE_OVS_INTERNAL:
			...
			if (!ecm_interface_mac_addr_get_no_route(dev, addr, node_addr)) {
				...
				if (!ecm_ipv6_dev_has_ipaddr(dev) && (ecm_front_end_is_bridge_port(dev))) {
					...
					dev_put(master);
				} else if (ecm_interface_is_ns_allowed(dev, skb)) {
							...
							__be16 *layer4hdr, struct sk_buff *skb)
{
	 ...
	 * ecm_front_end_ipv6_interface_construct_set_and_hold() function to set the input interfaces
				...
				const struct nf_hook_state *nhs)
{
	 ...
	 */
	spin_lock_bh(&ecm_ipv6_lock);
	if (unlikely(ecm_front_end_ipv6_stopped) || unlikely(atomic_read(&ecm_front_end_ipv6_stopped_temp))) {
		spin_unlock_bh(&ecm_ipv6_lock);
