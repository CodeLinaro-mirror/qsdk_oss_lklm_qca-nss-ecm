	...
	for (list_index = from_ifaces_first; list_index < ECM_DB_IFACE_HEIRARCHY_MAX; list_index++) {
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
				...
				rule_invalid = true;
				DEBUG_TRACE("%px: DSA/VLAN - Q-in-Q vlan unsupported\n", feci);
				ecm_sfe_stats_v6_inc(feci, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST_FROM_IFACE_DSA_QINQ_UNSUPPORTED);
				break;
			}
			ecm_db_iface_dsa_info_get(ii, &dsa_info);
			dsa_vlan_value = ((dsa_info.vlan_tpid << 16) | dsa_info.vlan_tag);
			 ...
			 */
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] == 0) {
				create->vlan_primary_rule.ingress_vlan_tag = dsa_vlan_value;
			} else {
				create->vlan_secondary_rule.ingress_vlan_tag = dsa_vlan_value;
			}
			create->valid_flags |= SFE_RULE_CREATE_VLAN_VALID;
			interface_type_counts[ECM_DB_IFACE_TYPE_VLAN]++;
...
#else
			rule_invalid = true;
			ecm_sfe_stats_v6_inc(feci, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST_FROM_IFACE_DSA_UNSUPPORTED);
		...
		for (list_index = *to_ii_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
			...
			case ECM_DB_IFACE_TYPE_DSA:
				...
				if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
					...
					rule_invalid = true;
					DEBUG_TRACE("%px: DSA/VLAN - Q-in-Q vlan unsupported\n", feci);
					ecm_sfe_stats_v6_inc(feci, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST, ECM_SFE_STATS_V6_EXCEPTION_MULTICAST_TO_IFACE_DSA_QINQ_UNSUPPORTED);
					break;
				}
				ecm_db_iface_dsa_info_get(ii, &dsa_info);
				dsa_vlan_value = ((dsa_info.vlan_tpid << 16) | dsa_info.vlan_tag);
