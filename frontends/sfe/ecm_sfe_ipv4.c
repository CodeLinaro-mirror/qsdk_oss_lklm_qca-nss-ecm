...
static struct ctl_table_header *ecm_sfe_ipv4_ctl_table_header;	/* Sysctl table header */
...
/*
 * ecm_sfe_ipv4_accel_limit_mode_handler()
 * 	Proc handler to limit the number of connection
 */
static int ecm_sfe_ipv4_accel_limit_mode_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To limit the number of acceleration
	 * echo 1 > /proc/sys/net/ecm/ecm_sfe_ipv4/accel_limit_mode
	 *
	 * To disable acceleration limitations
	 * echo 0 > /proc/sys/net/ecm/ecm_sfe_ipv4/accel_limit_mode
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_sfe_ipv4/accel_limit_mode
	 */
	 ...
	 */
	current_val = ecm_sfe_ipv4_accel_limit_mode;
	 ...
	 */
	if (ecm_sfe_ipv4_accel_limit_mode & ~(ECM_FRONT_END_ACCEL_LIMIT_MODE_FIXED | ECM_FRONT_END_ACCEL_LIMIT_MODE_UNLIMITED)) {
		DEBUG_ERROR("ecm_sfe_ipv4_accel_limit_mode = %x bad\n", ecm_sfe_ipv4_accel_limit_mode);
		ecm_sfe_ipv4_accel_limit_mode = current_val;
		return -EINVAL;
	}
...
/*
 * ecm_sfe_ipv4_driver_fail_limit_default_handler()
 * 	Proc handler to limit driver interactions failure
 */
static int ecm_sfe_ipv4_driver_fail_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To limit number of SFE interaction failure (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_sfe_ipv4/driver_fail_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_sfe_ipv4/driver_fail_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_sfe_ipv4_driver_fail_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_sfe_ipv4_driver_fail_limit_default < 0) {
		ecm_sfe_ipv4_driver_fail_limit_default = current_val;
...
/*
 * ecm_sfe_ipv4_nack_limit_default_handler()
 * 	Proc handler to handle nack limit
 */
static int ecm_sfe_ipv4_nack_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To handle number of nack from SFE (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_sfe_ipv4/nack_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_sfe_ipv4/nack_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_sfe_ipv4_nack_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_sfe_ipv4_nack_limit_default < 0) {
		ecm_sfe_ipv4_nack_limit_default = current_val;
...
/*
 * ecm_sfe_ipv4_no_action_limit_default_handler()
 * 	Proc handler to set default no action limit
 */
static int ecm_sfe_ipv4_no_action_limit_default_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * To set number of no action seen from frontend (x >= 0)
	 * echo x > /proc/sys/net/ecm/ecm_sfe_ipv4/no_action_limit_default
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_sfe_ipv4/no_action_limit_default
	 */
	 ...
	 * Write the val with user input
	 */
	current_val = ecm_sfe_ipv4_no_action_limit_default;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if (ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if (ecm_sfe_ipv4_no_action_limit_default < 0) {
		ecm_sfe_ipv4_no_action_limit_default = current_val;
		DEBUG_ERROR("Invalid input, valid input positive integer\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_XFRM
/*
 * ecm_sfe_ipv4_reject_acceleration_for_ipsec_handler()
 * 	Proc handler to enable/diable IPsec acceleration
 */
static int ecm_sfe_ipv4_reject_acceleration_for_ipsec_handler(ECM_CTL_TABLE_CONST struct ctl_table *ctl, int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	/*
	 * Usage:
	 *
	 * Enable IPsec acceleration
	 * echo 1 > /proc/sys/net/ecm/ecm_sfe_ipv4/reject_acceleration_for_ipsec
	 *
	 * Disable IPsec acceleration
	 * echo 0 > /proc/sys/net/ecm/ecm_sfe_ipv4/reject_acceleration_for_ipsec
	 *
	 * To read status
	 * cat /proc/sys/net/ecm/ecm_sfe_ipv4/reject_acceleration_for_ipsec
	 */

	int ret;
	int current_val;

	current_val = ecm_sfe_ipv4_reject_acceleration_for_ipsec;
	ret = proc_dointvec(ctl, write, buffer, lenp, ppos);
	if(ret || (!write)) {
		/*
		 * Return if failure or read operation
	...
	if ((ecm_sfe_ipv4_reject_acceleration_for_ipsec != 0) && (ecm_sfe_ipv4_reject_acceleration_for_ipsec != 1)) {
		ecm_sfe_ipv4_reject_acceleration_for_ipsec = current_val;
...
static struct ctl_table ecm_sfe_ipv4_ctl_table[] = {
	{
		.procname	= "accel_limit_mode",
		.data		= &ecm_sfe_ipv4_accel_limit_mode,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_sfe_ipv4_accel_limit_mode_handler,
	},
	{
		.procname	= "driver_fail_limit_default",
		.data		= &ecm_sfe_ipv4_driver_fail_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_sfe_ipv4_driver_fail_limit_default_handler,
	},
	{
		.procname	= "nack_limit_default",
		.data		= &ecm_sfe_ipv4_nack_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_sfe_ipv4_nack_limit_default_handler,
	},
	{
		.procname	= "no_action_limit_default",
		.data		= &ecm_sfe_ipv4_no_action_limit_default,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_sfe_ipv4_no_action_limit_default_handler,
	},
#ifdef CONFIG_XFRM
	{
		.procname	= "reject_acceleration_for_ipsec",
		.data		= &ecm_sfe_ipv4_reject_acceleration_for_ipsec,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= &ecm_sfe_ipv4_reject_acceleration_for_ipsec_handler,
	},
#endif
};
...
int ecm_sfe_ipv4_init(struct dentry *dentry)
{
	...
	ecm_sfe_ipv4_ctl_table_header = register_sysctl(ECM_SFE_IPV4_PATH, ecm_sfe_ipv4_ctl_table);
	if (!ecm_sfe_ipv4_ctl_table_header) {
		DEBUG_ERROR("Failed to create ecm sfe ipv4 directory in sysctl\n");
		return -1;
	}

	if (!ecm_debugfs_create_dir("ecm_sfe_ipv4", dentry, &ecm_sfe_ipv4_dentry)) {
		DEBUG_ERROR("Failed to create ecm sfe ipv4 directory in debugfs\n");
		unregister_sysctl_table(ecm_sfe_ipv4_ctl_table_header);
		return -1;
	}

	ecm_debugfs_create_u32("accelerated_count", S_IRUGO, ecm_sfe_ipv4_dentry,
				(u32 *)&ecm_sfe_ipv4_accelerated_count);
	ecm_debugfs_create_u32("pending_accel_count", S_IRUGO, ecm_sfe_ipv4_dentry,
				(u32 *)&ecm_sfe_ipv4_pending_accel_count);
	ecm_debugfs_create_u32("pending_decel_count", S_IRUGO, ecm_sfe_ipv4_dentry,
				(u32 *)&ecm_sfe_ipv4_pending_decel_count);

	if (!ecm_debugfs_create_file("accel_cmd_avg_millis", S_IRUGO, ecm_sfe_ipv4_dentry,
	...
	if (!ecm_debugfs_create_file("decel_cmd_avg_millis", S_IRUGO, ecm_sfe_ipv4_dentry,
	...
	ecm_sfe_ported_ipv4_init(ecm_sfe_ipv4_dentry);

	if (!ecm_debugfs_create_file("stats_request_counter", S_IRUGO, ecm_sfe_ipv4_dentry,
...
#ifdef ECM_NON_PORTED_SUPPORT_ENABLE
	ecm_sfe_non_ported_ipv4_init(ecm_sfe_ipv4_dentry);
#endif
...
task_cleanup:
	...
	ecm_debugfs_remove_recursive(ecm_sfe_ipv4_dentry);
	unregister_sysctl_table(ecm_sfe_ipv4_ctl_table_header);
	return -1;
}
