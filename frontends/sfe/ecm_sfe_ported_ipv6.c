	...
	for (list_index = from_ifaces_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
				...
				ecm_sfe_stats_v6_inc(feci, ECM_SFE_STATS_V6_EXCEPTION_PORTED, ECM_SFE_STATS_V6_EXCEPTION_PORTED_FROM_IFACE_DSA_QINQ_UNSUPPORTED);
				break;
			}
	...
	for (list_index = to_ifaces_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			if (interface_type_counts[ECM_DB_IFACE_TYPE_VLAN] > 0) {
				rule_invalid = true;
				DEBUG_TRACE("%px: DSA/VLAN - Q-in-Q vlan unsupported\n", feci);
				ecm_sfe_stats_v6_inc(feci, ECM_SFE_STATS_V6_EXCEPTION_PORTED, ECM_SFE_STATS_V6_EXCEPTION_PORTED_TO_IFACE_DSA_QINQ_UNSUPPORTED);
				break;
			}
