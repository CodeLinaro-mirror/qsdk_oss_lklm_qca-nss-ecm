									...
									struct nf_conn *ct, struct sk_buff *skb)
{
	...
	for (list_index = from_ifaces_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			ecm_sfe_stats_v6_inc(ECM_SFE_STATS_V6_EXCEPTION_PORTED, ECM_SFE_STATS_V6_EXCEPTION_PORTED_FROM_IFACE_DSA_UNSUPPORTED);
#endif
			break;
	...
	for (list_index = to_ifaces_first; !rule_invalid && (list_index < ECM_DB_IFACE_HEIRARCHY_MAX); list_index++) {
		...
		case ECM_DB_IFACE_TYPE_DSA:
			...
			rule_invalid = true;
			DEBUG_TRACE("%px: DSA interface is not supported\n", feci);
			ecm_sfe_stats_v6_inc(ECM_SFE_STATS_V6_EXCEPTION_PORTED, ECM_SFE_STATS_V6_EXCEPTION_PORTED_TO_IFACE_DSA_UNSUPPORTED);
#endif
			break;
