...
static void ecm_classifier_emesh_sawf_flowsawf_set(struct ecm_front_end_flowsawf_msg *msg)
{
	...
	if (!aci) {
		DEBUG_WARN("emesh classifier is not assigned. ci=%px %u\n", ci, ci->serial);
		goto end;
	}
						...
						uint8_t fse_flags)
{
	...
	if (cemi->process_response.flow_service_class)
		fse_info.fw_svc_info = cemi->process_response.flow_service_class;

	if (cemi->process_response.return_service_class)
 ...
 */
void ecm_classifier_emesh_sawf_params_sync_on_conn_decel(struct ecm_classifier_instance *aci, struct ecm_classifier_rule_sync *sync)
{
	struct ecm_classifier_emesh_sawf_instance *cemi;
	struct ecm_db_connection_instance *ci;
