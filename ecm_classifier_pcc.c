...
/*
 * ecm_classifier_pcc_decel_by_mac_addr()
 *	Decelerate connection based on a mac_addr.
 */
void ecm_classifier_pcc_decel_by_mac_addr(uint8_t *mac_addr)
{
	if (!mac_addr)
		return;
...
EXPORT_SYMBOL(ecm_classifier_pcc_decel_by_mac_addr);

/*
 * ecm_classifier_pcc_decel_by_ip_addr_v4()
 *	Decelerate connection based on a ip_addr.
 */
void ecm_classifier_pcc_decel_by_ip_addr_v4(__be32 ip_addr)
{
	ip_addr_t ecm_ip_addr;

	ECM_NIN4_ADDR_TO_IP_ADDR(ecm_ip_addr, ip_addr);
	ecm_db_host_connections_defunct_by_dir(ecm_ip_addr, ECM_DB_OBJ_DIR_FROM);
	ecm_db_host_connections_defunct_by_dir(ecm_ip_addr, ECM_DB_OBJ_DIR_TO);
}
EXPORT_SYMBOL(ecm_classifier_pcc_decel_by_ip_addr_v4);

/*
 * ecm_classifier_pcc_decel_by_ip_addr_v6()
 *	Decelerate connection based on a ip_addr.
 */
void ecm_classifier_pcc_decel_by_ip_addr_v6(struct in6_addr ip6_addr)
{
	ip_addr_t ecm_ip_addr;

	ECM_NIN6_ADDR_TO_IP_ADDR(ecm_ip_addr, ip6_addr);
	ecm_db_host_connections_defunct_by_dir(ecm_ip_addr, ECM_DB_OBJ_DIR_FROM);
	ecm_db_host_connections_defunct_by_dir(ecm_ip_addr, ECM_DB_OBJ_DIR_TO);
}
EXPORT_SYMBOL(ecm_classifier_pcc_decel_by_ip_addr_v6);
